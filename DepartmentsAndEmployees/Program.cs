...
namespace DepartmentsAndEmployees
{
    class Program
    {
        ...
        public static void PrintDepartmentReport(string title, List<Department> d)
        {
            Console.WriteLine(title);
            for (int i = 0; i < d.Count; i++)
            { Console.WriteLine($"{i+1}. {d[i].DeptName}");
            }
        }
        ...
        public static void PrintEmployeeReport(string title, List<Employee> e)
        {
            Console.WriteLine(title);
            for (int i = 0; i < e.Count; i++) {
                    if (e[i].Department == null)
                    { Console.WriteLine($"{i+1}. {e[i].FirstName} {e[i].LastName}"); }
                    else { Console.WriteLine($"{i+1}. {e[i].FirstName} {e[i].LastName}: Dept. Name {e[i].Department.DeptName}"); };
            }
